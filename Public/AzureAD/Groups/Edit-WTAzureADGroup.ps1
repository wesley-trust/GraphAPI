<#
.Synopsis
    Update Azure AD groups deployed in the Azure AD tenant
.Description
    This function updates the Azure AD groups in Azure AD using the Microsoft Graph API.
    The following Microsoft Graph API permissions are required for the service principal used for authentication:
        Group.ReadWrite.AzureAD
        Group.Read.All
        Directory.Read.All
        Agreement.Read.All
...
.PARAMETER AzureADGroups
    The Azure AD groups to remove, a group must have a valid id
.INPUTS
    None
.OUTPUTS
    None
...
.Example
    ...
    Edit-WTAzureADGroup @Parameters -RemoveAllExistingGroups
    $AzureADGroups | Edit-WTAzureADGroup -AccessToken $AccessToken
#>
