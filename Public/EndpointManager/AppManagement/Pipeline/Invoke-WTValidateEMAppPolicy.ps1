function Invoke-WTValidateEMAppPolicy {
    [CmdletBinding()]
    param (
        [parameter(
            Mandatory = $false,
        ...
        [parameter(
            ...
            HelpMessage = "The Endpoint Manager App Policies to be validated if not imported from a JSON file"
        )]
    ...
    Process {
        try {
            ...
            # Import App policies from JSON file, if the files exist
            if ($FilePath) {
                $EMAppPolicyImport = foreach ($File in $FilePath) {
                ...
                if ($EMAppPolicyImport) {
                    $EMAppPolicies = $EMAppPolicyImport | ConvertFrom-Json
                }
                else {
            ...
            if ($EMAppPolicies) {
                ...
                Write-Host "Importing Endpoint Manager App Policies"
                Write-Host "Policies: $($EMAppPolicies.count)"
                
                foreach ($Policy in $EMAppPolicies) {
                ...
                if ($ImportOnly) {
                    ...
                    # Output current action
                    Write-Host "Validating Endpoint Manager App Policies"
    
                    # For each policy, run validation checks
                    $InvalidPolicies = foreach ($Policy in $EMAppPolicies) {
                        $PolicyValidate = $null
                    ...
                    if ($InvalidPolicies) {
                        Write-Host "Invalid Policies: $($InvalidPolicies.count) out of $($EMAppPolicies.count) imported"
                ...
                $ErrorMessage = "No App policies to be imported, import may have failed or none may exist"
                throw $ErrorMessage
            }
