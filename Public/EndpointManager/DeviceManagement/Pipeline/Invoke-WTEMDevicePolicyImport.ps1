function Invoke-WTEMDevicePolicyImport {
    [CmdletBinding()]
    param (
        [parameter(
            Mandatory = $false,
        ...
        [Parameter(
            Mandatory = $false,
            ValueFromPipeLineByPropertyName = $true,
            HelpMessage = "Specify whether the groups used for EMDevice policies, should not be removed, if the policy is removed"
        )]
        [switch]
        $ExcludeGroupRemoval,
        [parameter(
    ...
    Begin {
        try {
            ...
            $Functions = @(
                ...
                "GraphAPI\Public\EndpointManager\DeviceManagement\Pipeline\Invoke-WTValidateEMDevicePolicy.ps1",
                "GraphAPI\Public\EndpointManager\DeviceManagement\Pipeline\Invoke-WTPlanEMDevicePolicy.ps1",
                "GraphAPI\Public\EndpointManager\DeviceManagement\Pipeline\Invoke-WTApplyEMDevicePolicy.ps1"
            )
        ...
        try {
            ...
            if ($Stage -eq "Plan" -or $Stage -eq "Apply") {
                ...
                if ($AccessToken) {
                    ...
                    if ($ValidateEMDevicePolicies) {
                        $PlanParameters.Add("EMDevicePolicies", $ValidateEMDevicePolicies)
                    }
                    ...
                    Invoke-WTPlanEMDevicePolicy @PlanParameters | Tee-Object -Variable PlanEMDevicePolicies
                ...
                if ($Stage -eq "Apply") {
                    if ($PlanEMDevicePolicies) {
                        ...
                        Invoke-WTApplyEMDevicePolicy @ApplyParameters
                    }
                    else {
