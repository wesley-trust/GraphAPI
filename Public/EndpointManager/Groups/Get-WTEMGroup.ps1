    ...
    Process {
        try {
            ...
            if ($AccessToken) {
                ...
                $EndpointManagerGroups = Get-WTAzureADGroup @Parameters | Where-Object {$_.$Tag -eq $Service}
                
                # Return response if the filtered objects contain the specified service
                if ($EndpointManagerGroups) {
                    $EndpointManagerGroups
                }
                else {
                    $WarningMessage = "No Endpoint Manager groups exist in Azure AD, or with the specified IDs"
